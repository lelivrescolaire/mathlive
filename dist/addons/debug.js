...
exports.latexToAsciiMath = latexToAsciiMath;
exports.asciiMathToLatex = asciiMathToLatex;
exports.default = void 0;

var _outputASCIIMath = require("../editor/outputASCIIMath.js");

var _lexer = _interopRequireDefault(require("../core/lexer.js"));

var _parser = _interopRequireDefault(require("../core/parser.js"));

var _editorEditableMathlist = require("../editor/editor-editableMathlist.js");
 ...
 * @private
 */
function latexToAsciiMath(latex, mode) {
  mode = mode || 'math';

  var mathlist = _parser.default.parseTokens(_lexer.default.tokenize(latex), mode, null, null);

  return (0, _outputASCIIMath.toASCIIMath)(mathlist);
}

function asciiMathToLatex(ascii) {
  return (0, _editorEditableMathlist.parseMathString)(ascii, {
    format: 'ASCIIMath'
  });
}
/**
...
function hasClass(spans, symbol, cls) {
  var classes = getClasses(spans, symbol);
...
function mathlistToString(mathlist, indent) {
  ...
  if (Array.isArray(mathlist)) {
    ...
    result += mathlistPropToString(mathlist, 'fontFamily', indent); // Type 'genfrac'
    ...
    result += mathlistPropToString(mathlist, 'denom', indent); // Type...?

    result += mathlistPropToString(mathlist, 'limits', indent);
    result += mathlistPropToString(mathlist, 'symbol', indent); // Type 'color'

    result += mathlistPropToString(mathlist, 'framecolor', indent); // Type 'line'

    result += mathlistPropToString(mathlist, 'position', indent); // Type 'mathstyle'

    result += mathlistPropToString(mathlist, 'mathstyle', indent); // Common
...
function spanToMarkup(span, indent) {
  indent = indent || ''; // if (indent.length === 0) {
  //     result += '<table>';
  // }
  ...
  if (Array.isArray(span)) {
    ...
    for (var _i = 0; _i < span.array.length; _i++) {
      for (var j = 0; j < span.array[_i].length; j++) {
        result += '<br>' + indent + '[' + (_i + 1) + ', ' + (j + 1) + '] ';
        result += spanToMarkup(span.array[_i][j], '');
      }
    }
  }
...
function mathlistToMarkup(mathlist, indent) {
  ...
  if (Array.isArray(mathlist)) {
    ...
    if (mathlist.array) {
      for (var _i2 = 0; _i2 < mathlist.array.length; _i2++) {
        result += '<br>' + indent + "\u2317 row " + (_i2 + 1) + '/' + mathlist.array.length;

        for (var j = 0; j < mathlist.array[_i2].length; j++) {
          result += mathlistToMarkup(mathlist.array[_i2][j], indent + "\u2317\u232A");
        }
      }
    }
  }
...
var _default = {
  ...
  getType: getType,
  getTag: getTag,
  latexToAsciiMath: latexToAsciiMath,
  asciiMathToLatex: asciiMathToLatex
};
