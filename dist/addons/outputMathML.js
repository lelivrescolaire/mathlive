...
function scanOperator(stream, final, options) {
  ...
  if (stream.index < final && (atom.type === 'mbin' || atom.type === 'mrel')) {
    ...
    if (atom.limits === 'limits' && (atom.superscript || atom.subscript)) {
      ...
      var _op = isUnit ? '<mi class="MathML-Unit"' + makeID(_atom.id, options) + '>' + toString(_atom.body) + '</mi>' : toMo(_atom, options);
...
_mathAtom.default.MathAtom.prototype.toMathML = function (options) {
  ...
  switch (this.type) {
    ...
    case 'box':
      ...
      if (this.backgroundcolor) {
        result += ' mathbackground="' + _color.default.stringToColor(this.backgroundcolor) + '"';
      }
...
_mathAtom.default.toMathML = function (atoms, options) {
